...
const Navbar = () => {
  ...
  return (
    ...
    <NavLink to={'/'} onClick={()=>setOpen(false)}>
       <h3 className='text-2xl md:text-3xl'>Fresh<span className='text-orange-700 font-bold text-2xl md:text-4xl '>Store</span></h3>
    </NavLink>
 ...
 { open &&(
      <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-20`}>
       ...
       {
        !user ? (
          <button onClick={()=>{setOpen(false);setShowUserLogin(true)}}  className="cursor-pointer px-6 py-2 mt-2 hover:bg-primary-dull bg-primary transition text-white rounded-full text-sm">
              Login
          </button>
        ):
        <button onClick={logout} className="cursor-pointer px-6 py-2 mt-2 hover:bg-primary-dull bg-primary transition text-white rounded-full text-sm">
              Logout
          </button>
       }
